//npm install -g uglify-js xmldom xpath htmlparser node-minify esprima jsdom@9.12.0 csso jsonlint canvas node-pre-gyp install postinstall-build
//node compress.js
//node compress.js ..\public\ohumlite\www\
//const niv = require('npm-install-version');
//niv.install('jsdom@9.12.0');

const runtimeRequire = eval("require");
const jsonlint = runtimeRequire('jsonlint');
const fs = require('fs');
...
const gcc = runtimeRequire('@node-minify/google-closure-compiler');
	...
	if (flag) {
		...
		try {
			...
			if (is('String', fb)) {
				...
				console.log("XXXXX Error compiling template " + tname);
				console.log(fb);
			} else {
				fb = fb.toString();
			...
			console.log("XXXXX Exception compiling template " + tname);
			console.log(err);
			console.log(currtemcode);
		}
	}
}
...
function compressJsCss(rawHtml, config, dirPath, schemas, htmlTemplates, module) {
    let doc = new dom({
        errorHandler:{warning:function(w){},error: function(e){console.error(e);throw e;},fatalError: function(e){console.error(e);throw e;}}
    ...
    function validateJsFiles(jsFileObjArr, cssFiles, config, dirPath) {
        ...
        for(let i = 0; i < jsFileObjArr.length; i++) {
         ...
         // console.log(JSON.stringify(jsfilesAll));
        // console.log(JSON.stringify(jsFiles));

        console.log("Total number of js files = " + jsFileObjArr.length);
        console.log("Total number of css files = " + (cssFiles[0].length + cssFiles[1].length));

        let flag = 0;
        let allcssfiles = [].concat(cssFiles[0]).concat(cssFiles[1]);
        for (let i = 0; i < allcssfiles.length; i++) {
            //console.log(allcssfiles[i]);
            let c = fs.readFileSync(allcssfiles[i], 'utf8');
            try {
                let ast = csso.syntax.parse(c);
            } catch (err) {
                flag = 2;
                console.log("XXXXX Got (" + err + ") while parsing " + allcssfiles[i]);
            }
        }
            ...
            try {
                ...
                for (let j = 0; j < tree.body.length; j++) {
                    if (tree.body[j].type == 'FunctionDeclaration') {
                        ...
                        if (fmap[fkk]) {
                            flag = 1;
                            console.log("XXXXX Duplicate FunctionDeclaration function " + fkk + " in file " + jsfilesAll[i])
                        } else {
                            fmap[fkk] = true;
                        }
                    }
                ...
                if((e+"").indexOf("XXXXX Error:")!=-1) {
                    console.log((e + "").replace("XXXXX Error:", "Error in file " + jsfilesAll[i] + ":"));
                } else {
                    if((e+"").indexOf("\n")!=-1) {
                        console.log("Error in file " + jsfilesAll[i] + ": " + (e+"").split("\n")[0]);
                    } else {
                        console.log("Error in file " + jsfilesAll[i] + ": ");
                        console.log(e);
                    }
                }
                // console.log("***Syntax errors found in file " + jsfilesAll[i] +
                // ", Error is " + err);
                flag = 2;
                break;
            }
                ...
                callback: function(ind) {
                    return function(err, min) {
                        if(min){}
                            ...
                            console.log("Problem compacting js file " + config.modules[ind]);
                        }
                    };
                }(i)
            });
    ...
    function processMinify(jsFileObjArr, cssFiles, config, dirPath) {
		...
		try {
			...
			console.log("Compaction of js files successfull");
			cntxt = 3;
			...
			console.log("Compression of css files successfull");
			//compressConfig();
		} catch (err) {
			let e;
...
function compress(dirPath, module, fileName, type, cb, lazyValidation, htmlTemplates) {
	...
	function compressConfig(config, fileName, schemas, htmlTemplates, type, module) {
        ...
        if(tiuschs.length>0) {
            console.log("Found only "+tiuschs.length+ " usable tiuxmls, will compress only these");
        }
        ...
        if(Object.keys(schemas).length>0) {
            if(tiuschs.length>0) {
                ...
                if(fl>0) {
                    console.log("Updated total number of schemas to be compressed = " + fl);
                    schemas = dschemas;
                }
            }
            if(fileName) {
                console.log("Changed/New schema file is " + fileName);
                ...
                console.log("Generating compressed schema file for all schemas");
                if(module != "") {
        ...
        if(Object.keys(htmlTemplates).length>0) {
            if(tiutmps.length>0) {
                ...
                if(fl>0) {
                    console.log("Updated total number of templates to be compressed = " + fl);
                    htmlTemplates = dhtmlTemplates;
                }
            }
            if(fileName) {
                console.log("Changed/New template file is " + fileName);
                ...
                console.log("Generating compressed template file for all templates");
                if(module != "") {
    ...
    if(type!==2) {
        ...
        console.log("Total number of schemas to be compressed = " + sc[0]);
    }
    ...
    if(type!=1) {
    	...
    	console.log("Total number of templates to be compressed = " + tc[0]);
    }
